          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <form id="update-qty-form" action="{{url('cart', $item->idProduk)}}" method="POST">
                    @csrf
                    <div class="input-group mb-3" style="max-width: 120px;">
                      <div class="input-group-prepend">
                        <button class="btn btn-outline-primary js-btn-minus" type="button">&minus;</button>
                      </div>
                      <input type="text" class="form-control text-center qty-input" value="{{ $item->qty }}" placeholder="" aria-label="Example text with button addon" aria-describedby="button-addon1" data-id="{{ $item->idProduk }}">
...
<script>
  $(document).ready(function() {
    ...
    $(".js-btn-plus").on("click", function() {
      ...
      $.ajax({
        url: "/update-cart/" + productId + "/increase",
        type: "POST",
        data: {
          _token: "{{ csrf_token() }}"
        },
    ...
    $(".js-btn-minus").on("click", function() {
      ...
      $.ajax({
        url: "/update-cart/" + productId + "/decrease",
        type: "POST",
        data: {
          _token: "{{ csrf_token() }}"
        },
